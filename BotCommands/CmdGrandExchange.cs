  ...
  class CmdGrandExchange {
    ...
    public static void Price(CommandContext bc) {
      ...
      for (int i = 0; i < Math.Min(10, price_list.Count); i++) {
        reply += string.Format(" | {0}: \\c07{1}", price_list[i].Name, Util.FormatShort(qty * price_list[i].MarketPrice, 2));
        reply += "\\c";
        if (price_list[i].ChangeToday > 0)
          reply += string.Format(" \\c3[+{0}]\\c", Util.FormatShort(price_list[i].ChangeToday, 1));
        else if (price_list[i].ChangeToday < 0)
          reply += string.Format(" \\c4[{0}]\\c", Util.FormatShort(price_list[i].ChangeToday, 1));
      }
