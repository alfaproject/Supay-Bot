...
namespace Supay.Bot {
  static partial class Command {
    ...
    public static void Event(CommandContext bc) {
      try {
        bool all = false;
        if (bc.Message.Contains("@all")) all = true;
        DateTime startTime = DateTime.UtcNow;
        string eventPage = new System.Net.WebClient().DownloadString("http://supremeskillers.net/api/?module=events&action=getNext");
        ...
        if (!all) {
          ...
          bc.SendReply("Next event: \\c07{0}\\c starts in \\c07{1}\\c for more information: \\c12{2}\\c"
            .FormatWith(
              (string)nextEvent["description"],
              (startTime - DateTime.UtcNow).ToLongString(),
              (string)nextEvent["url"])
            );
          ...
          for (i = 0; i < 10; i++) {
            ...
            if (i == 0)
              ...
              reply += "{0} (\\c07{1}\\c); "
                .FormatWith(
                  (string)nextEvent["description"],
                  String.Format("{0: dddd d MMMM}", startTime, CultureInfo.InvariantCulture).Trim()
                );
          }
